...
func (s *Server) apiDisconnectListener(w http.ResponseWriter, r *http.Request) {
	...
	if !s.hasAccess(user, listener.CurrentMountName()) {
		jsonError(w, "Forbidden", http.StatusForbidden)
		return
	}
...
func (s *Server) apiMoveListener(w http.ResponseWriter, r *http.Request) {
	...
	if !s.hasAccess(user, listener.CurrentMountName()) || !s.hasAccess(user, body.TargetMount) {
		jsonError(w, "Forbidden", http.StatusForbidden)
		return
	}
	...
	if body.TargetMount == listener.CurrentMountName() {
		jsonResponse(w, map[string]string{"status": "ok"})
		return
	}
