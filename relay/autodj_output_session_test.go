...
func TestActivateTrackSourcePublishesMetadataAfterTrackFrameIsWritten(t *testing.T) {
	r := NewRelay(false, nil)
	streamer := &Streamer{
		Name:           "Gap Filler",
		OutputMount:    "/gap",
	...
	source, err := sm.activateTrackSource(context.Background(), streamer, writeTestMP3File(t), 0, 1)
	if err != nil {
		t.Fatalf("activateTrackSource: %v", err)
	}
	defer source.Close()

	select {
	case evt := <-metaCh:
		t.Fatalf("metadata emitted before output session started the track: %+v", evt)
	default:
	}
